...
class Touch {
  ...
  send(identifier = 0, touch = 0, x = 0, y = 0) {
    ...
    let dummyData = [
      ...
      identifier,       // contact identifier
      touch,            // finger touches display
      this._MSB(x + 1), this._LSB(x + 1), // x
      this._MSB(y + 1), this._LSB(y + 1), // y
      0x00,
    ];

    if (this.enabled && contact > 1 && contact > this.prevContact) {
      this.self.write(dummyData);
    }
